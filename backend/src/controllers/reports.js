...
export async function monthlySummary(req, res) {
  ...
  const params = [String(year), req.user.id];
  const where = ["TO_CHAR(t.date, 'YYYY') = $1", 'a.user_id = $2'];
...
export async function categoryBreakdown(req, res) {
  ...
  const params = [type, String(year), req.user.id];
  const conditions = ['t.type = $1', "TO_CHAR(t.date, 'YYYY') = $2", 'a.user_id = $3'];
...
export async function dashboardStats(req, res) {
  ...
  const [current, prev, balances, recent, topExp] = await Promise.all([
    pool.query(`
      ...
      FROM transactions t
      JOIN accounts a ON a.id = t.account_id
      WHERE a.user_id = $1 AND TO_CHAR(t.date, 'MM') = $2 AND TO_CHAR(t.date, 'YYYY') = $3
      GROUP BY a.currency
    `, [userId, month, year]),
...
export async function last30DaysTrend(req, res) {
  ...
  const where  = ["t.date >= CURRENT_DATE - INTERVAL '30 days'", 'a.user_id = $1'];
